...
try:
    sql = 'SELECT DATE, SUM(sales_qty) AS qty FROM sales WHERE product_id ='+str(p_id[0])+' GROUP BY DATE'
    cur.execute(sql)
    rows = cur.fetchall()
    count = cur.rowcount
    if (count > 59):
    # Continue only if there are rows returned.
        ...
        def gradientBoosting(df1, forecast_length, forecast_period):
            df3 = df1[['qty', 'DATE']]
            ...
            rfe = RFE(GradientBoostingRegressor(n_estimators=300,
                                                max_depth=5,
                                                learning_rate=0.01,
                                                min_samples_split=3))
        ...
        def calculate (database_name, pid, toEval, rc):
            ...
            for x in range(rc):
                ...
                if totalQty == 0:
                    break
                dividedQty = math.floor(totalQty/30)
                if dividedQty == 0:
                    holder.append(1)
                else: 
        ...
        predicted_value = gradientBoosting(dataset, rc, 'Month')
