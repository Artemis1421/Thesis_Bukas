	...
	if(isset($_POST['excelButton'])){
		...
		$html .= '
			    	<tr>
		                ...
		                <td></td>
		                <td>PHP '.
		                	number_foramt($total_formatted, 2)
		                .'</td>
		                <td>PHP '.
		                	number_foramt($total_formatted2, 2)
		                .'</td>
		                <td>'.$yr_month.'</td>
			    	</tr>    
		    	</tbody>
        ...
        $html .= '
                 ...
                 ';
                        if($result = mysqli_query($conn, "SELECT paymethod, sum(total_payment) FROM orders WHERE date >= '$date_start' AND date <= '$date_end' AND s.deleted != 1 GROUP BY paymethod")){
