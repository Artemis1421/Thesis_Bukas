...
?>
            ...
            <div class="col">
                <header>
                    <div class="col d-flex justify-content-between align-items-md-center">
                        ...
                        <div>
                            <form action='../records_page/records_annual.php' method="post">
                    ...
                    <div>
                        <form action="../records_page/export_annual.php" method="post">
                            <div role="group" class="btn-group">
                                <button class="btn btn-primary pdf <?php if(isset($_POST['date_id'])){echo "";}else{ echo "disabled";} ?>" type="button" onclick="generatePDFA()" style="background: #d94444;border-style: none;">PDF</button>
                    ...
                    <table class="table sortable" id="table_with_data">
                            ...
                            <tr>
                            	<?php 
                            		if(isset($_POST['date_id'])){
                                        ...
                                        $formatDate = substr($formatDate, 0, 5);
                                        $yr_month = substr($formatDate, 0, 4);

                                        $date_start = $formatDate.'01-01';
                                        $date_end = $formatDate.'12-31';

                                		$_SESSION['date_start'] = $date_start;
                                        $_SESSION['date_end'] = $date_end;
                                        $_SESSION['yr_month'] = $yr_month;

                                		$sales = join_sales_annually_table($date_start, $date_end);
		                        		foreach($sales as $sale):
                        		?>
                        		<script>
                        			var formDate = '<?=$yr_month?>';
                        		</script>
                        ...
                        <tfoot>
                            <tr id="report_tr">
                                ...
                                ?>
                                <td><?php echo $yr_month; ?></td>
                            </tr>
                            <tr id="report_tr">
                                <td colspan="8"><b>Payment Methods</td>
                            </tr>
                            <?php 
                                if($result = mysqli_query($conn, "SELECT paymethod, sum(total_payment) FROM orders WHERE date >= '$date_start' AND date <= '$date_end' GROUP BY paymethod")):
    ...
    <script src="assets/js/export_annually.js"></script>
