...
if (isset($_SESSION['id']) && isset($_SESSION['business_name'])){
	$dateReport = $_SESSION['dateReport'];
	...
	if(isset($_POST['excelButton'])){
		$query = "SELECT p.product_name, c.category_name, p.product_price, p.product_cprice, SUM(sales_qty) AS total_qty, SUM(p.product_price * s.sales_qty) AS total_selling_price, SUM(s.sales_qty * (p.product_price - p.product_cprice)) AS total_income_price, s.date, s.deleted
					...
					WHERE s.date = '$dateReport' AND s.deleted != 1 
        ...
        $html .= '
                 ...
                 ';
                        if($result = mysqli_query($conn, "SELECT paymethod, sum(total_payment) FROM orders WHERE date = '$date' AND deleted != 1 GROUP BY paymethod")){
                           ...
                           if($result1 = mysqli_query($conn, "SELECT s.id, s.deleted, s.order_id, s.date, s.product_id, s.sales_qty, o.id, o.paymethod, p.id, p.product_price, p.product_cprice, sum(s.sales_qty * (p.product_price - p.product_cprice)) AS 'total_income' FROM sales s LEFT JOIN products p ON s.product_id = p.id LEFT JOIN orders o ON s.order_id = o.id WHERE paymethod = '$paymethod' AND s.date ='$date' AND s.deleted != 1 ")){
		...
		header('Content-Disposition:attachment;filename='.$dateReport.'.xls');
		echo $html;
	}
	...
	if(isset($_POST['csvButton'])){
		...
		$fields2 = array('TOTAL', '', '', '', '', 'PHP '.number_format($total_formatted, 2), 'PHP '.number_format($total_formatted2, 2), $dateReport);
		fputcsv($f, $fields2, $delimiter);

		$fields3 = array('Payment Methods');
		fputcsv($f, $fields3, $delimiter);

		if($result = mysqli_query($conn, "SELECT paymethod, sum(total_payment) FROM orders WHERE date = '$dateReport' AND deleted != 1 GROUP BY paymethod")):
            while($row = mysqli_fetch_array($result)):
                $paymethod = $row['paymethod'];
		...
		header('Content-Type:application/xls');
		header('Content-Disposition:attachment;filename='.$dateReport.'.csv');
