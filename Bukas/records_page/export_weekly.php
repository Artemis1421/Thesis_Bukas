...
if (isset($_SESSION['id']) && isset($_SESSION['business_name'])){
	$date_start = $_SESSION['dateReportStart'];
    $date_end = $_SESSION['dateReportEnd'];
	...
	if(isset($_POST['excelButton'])){
		$query = "SELECT p.product_name, c.category_name, p.product_price, p.product_cprice, SUM(sales_qty) AS total_qty, SUM(p.product_price * s.sales_qty) AS total_selling_price, SUM(s.sales_qty * (p.product_price - p.product_cprice)) AS total_income_price, s.date, s.deleted
					...
					WHERE s.date BETWEEN '$date_start' AND '$date_end' AND s.deleted != 1
		...
		header('Content-Disposition:attachment;filename='.$date_start.' - '.$date_end.'.xls');
		echo $html;
	}	
	...
	if(isset($_POST['csvButton'])){
		...
		$fields2 = array('TOTAL', '', '', '', '', 'PHP '.number_format($total_formatted, 2), 'PHP '.number_format($total_formatted2, 2), $date_start.' - '.$date_end);
		...
		header('Content-Type:application/xls');
		header('Content-Disposition:attachment;filename='.$date_start.' - '.$date_end.'.csv');
