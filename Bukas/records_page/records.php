            ...
            <div class="col">
                <header>
                    <div class="col d-flex justify-content-between align-items-md-center">
                        ...
                        <div>
                            <form action='../records_page/records.php' method="post">
                    ...
                    <div>
                        <form action="../records_page/export_daily.php" method="post">
                            <div role="group" class="btn-group">
                                <button class="btn btn-primary pdf <?php if(isset($_POST['date_id'])){echo "";}else{ echo "disabled";} ?>" type="button" onclick="generatePDF()" style="background: #d94444;border-style: none;">PDF</button>
                                <button class="btn btn-primary csv <?php if(isset($_POST['date_id'])){echo "";}else{ echo "disabled";} ?>" type="submit" name="csvButton" style="background: #fdb750;border-style: none;">CSV</button>
                                <button class="btn btn-primary excel <?php if(isset($_POST['date_id'])){echo "";}else{ echo "disabled";} ?>" type="submit" name="excelButton" style="background: #4ecf63;border-style: none;">Excel</button>
                            </div>
                        </form>
                    </div>
                </header>
                ...
                <div class="table-responsive">
                    <table class="table sortable" id="table_with_data">
                            ...
                            <tr>
                            	<?php 
                            		if(isset($_POST['date_id'])){
                                		...
                                		$_SESSION['dateReport'] = $formatDate;

                                		$sales = join_sales_table($formatDate);
                        ...
                        <tfoot>
                            ...
                            <?php 
                                if($result = mysqli_query($conn, "SELECT paymethod, sum(total_payment) FROM orders WHERE date = '$date' AND deleted != 1 GROUP BY paymethod")):
                                    while($row = mysqli_fetch_array($result)):
                                        $paymethod = $row['paymethod'];
                                    if($result1 = mysqli_query($conn, "SELECT s.id, s.deleted, s.order_id, s.product_id, s.date, s.sales_qty, o.id, o.paymethod, p.id, s.product_price, p.product_cprice, sum(s.sales_qty * (s.product_price - p.product_cprice)) AS 'total_income' FROM sales s LEFT JOIN products p ON s.product_id = p.id LEFT JOIN orders o ON s.order_id = o.id WHERE paymethod = '$paymethod' AND s.date ='$date' AND s.deleted != 1")):
...
function liveUpdate(){
    const live_date = document.getElementById('')
}
</script>
